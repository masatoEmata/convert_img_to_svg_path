...
def transform_segments_together(path, transformation):
    ...
    for i, (sa, sb) in enumerate(path.joints()):
        if sa.end == sb.start:
            transformed_segs[i].end = transformed_segs[(
                i + 1) % len(path)].start
...
def rotate(curve, degs, origin=None):
    ...
    if isinstance(curve, Path):
        def transformation(seg): return rotate(seg, degs, origin=origin)
...
def translate(curve, z0):
    ...
    if isinstance(curve, Path):
        def transformation(seg): return translate(seg, z0)
        return transform_segments_together(curve, transformation)
    elif is_bezier_segment(curve):
...
def scale(curve, sx, sy=None, origin=0j):
    ...
    * If `sy` is not specified, it is assumed to be equal to `sx` and 
    ...
    if isinstance(curve, Path):
        def transformation(seg): return scale(seg, sx, sy, origin)
        return transform_segments_together(curve, transformation)
    elif is_bezier_segment(curve):
...
def transform(curve, tf):
    ...
    if isinstance(curve, Path):
        def transformation(seg): return transform(seg, tf)
        return transform_segments_together(curve, transformation)
...
class Arc(object):
            ...
            if (self.rotation == 0) and (self.radius.real == self.radius.imag) and (other_seg.rotation == 0) and (other_seg.radius.real == other_seg.radius.imag):
                ...
                for p in possible_inters:
                    ...
                    if (other_t is None) or (other_t < 0.0) or (other_t > 1.0):
                        continue
                    assert(np.isclose(self.point(self_t),
                           other_seg.point(other_t), rtol=0.0, atol=1e-6))
                    i = (self_t, other_t)
                    inters.append(i)
    ...
    def as_cubic_curves(self, curves=1):
        ...
        for i in range(curves):
            ...
            alpha = sin(slice_t) * \
                (sqrt(4 + 3 * pow(tan((slice_t) / 2.0), 2)) - 1) / 3.0
            ...
            p_c1 = (p_start.real + alpha * ePrimen1x) + \
                (p_start.imag + alpha * ePrimen1y) * 1j
            p_c2 = (p_end.real - alpha * ePrimen2x) + \
                (p_end.imag - alpha * ePrimen2y) * 1j
...
class Path(MutableSequence):
    ...
    def intersect(self, other_curve, justonemode=False, tol=1e-12):
        ...
        """
        path1 = self
        path2 = other_curve if isinstance(
            other_curve, Path) else Path(other_curve)
        assert path1 != path2
        ...
        if intersection_list:
            pts = [_seg1.point(_t1) for _T1, _seg1,
                   _t1 in list(zip(*intersection_list))[0]]
            indices2remove = []
        ...
        while elements:
            ...
            if command == 'M':
                ...
                if last_command not in 'CS':
                    ...
                    control1 = current_pos + \
                        current_pos - segments[-1].control2
